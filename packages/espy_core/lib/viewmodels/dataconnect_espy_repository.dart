...
class DataConnectEspyRepository implements EspyRepository {
  ...
  Future<void> createLocalizedBroadcast({required String title, required String message, String? country, String? region, String? city}) async {
    try {
      await _db.createLocalizedBroadcast(
        title: title,
        message: message,
      ).country(country).region(region).city(city).execute();
    } catch (e) {
      debugPrint("DataConnect createLocalizedBroadcast failed, falling back: $e");
  ...
  Stream<List<Map<String, dynamic>>> listSosNumbers({String? countryId, String? sectorId, String? categoryId}) {
    var builder = _db.listSosNumbers();
    if (countryId != null) builder = builder.countryId(countryId);
    if (sectorId != null) builder = builder.sectorId(sectorId);
    if (categoryId != null) builder = builder.categoryId(categoryId);

    return builder.ref().subscribe().map((snap) =>
      snap.data.sosNumbers.map((sos) => <String, dynamic>{
        'id': sos.id.toString(),
        'labelEn': sos.labelEn,
        'labelAr': sos.labelAr,
        'number': sos.number,
        'countryId': sos.country.id,
        'sectorId': sos.sector?.id,
        'categoryId': sos.category?.id,
      }).toList()
    ).onErrorReturnWith((e, s) => <Map<String, dynamic>>[]);
  }
  ...
  Future<void> upsertSosNumber(Map<String, dynamic> sosData) async {
    final builder = _db.upsertSosNumber(
      countryId: sosData['countryId'],
      labelEn: sosData['labelEn'],
      number: sosData['number'],
      isActive: sosData['isActive'] ?? true,
    );
    if (sosData['id'] != null) builder.id(sosData['id']);
    if (sosData['sectorId'] != null) builder.sectorId(sosData['sectorId']);
    if (sosData['categoryId'] != null) builder.categoryId(sosData['categoryId']);
    if (sosData['labelAr'] != null) builder.labelAr(sosData['labelAr']);
    await builder.execute();
  }

  @override
  Stream<List<Map<String, dynamic>>> listTokenPackages({UserRole? targetRole, bool? isActive}) {
    var builder = _db.listTokenPackages();
    if (targetRole != null) {
      builder = builder.targetRole(sdk.UserRole.values.byName(targetRole.toSql()));
    }
    if (isActive != null) builder = builder.isActive(isActive);

    return builder.ref().subscribe().map((snap) =>
      snap.data.tokenPackages.map((pkg) => <String, dynamic>{
        'id': pkg.id,
        'nameEn': pkg.nameEn,
        'nameAr': pkg.nameAr,
        ...
        'targetRole': pkg.targetRole.stringValue,
  ...
  Future<void> upsertTokenPackage(Map<String, dynamic> data) async {
    await _db.upsertTokenPackage(
      ...
      targetRole: sdk.UserRole.values.byName(UserRole.parse(data['targetRole']).toSql()),
      isActive: data['isActive'] ?? true,
    ).nameAr(data['nameAr']).discountRate(data['discountRate']?.toDouble()).execute();
  }

  @override
  Stream<List<Map<String, dynamic>>> listElementPricing() {
    return _db.listElementPricing().ref().subscribe().map((snap) =>
      snap.data.elementPricings.map((ep) => <String, dynamic>{
        'id': ep.id,
        'tokenCost': ep.tokenCost,
        'validityDays': ep.validityDays,
        'updatedAt': ep.updatedAt?.toDateTime(),
      }).toList()
    ).onErrorReturnWith((e, s) => <Map<String, dynamic>>[]);
  }
  ...
  Future<void> updateElementPricing(String id, int tokenCost, {int? validityDays}) async {
    final builder = _db.updateElementPricing(id: id, tokenCost: tokenCost);
    if (validityDays != null) builder.validityDays(validityDays);
    await builder.execute();
  }
  ...
  Stream<List<Map<String, dynamic>>> listBroadcastModerationQueue({ModerationStatus status = ModerationStatus.pending}) {
    final sdkStatus = sdk.ModerationStatus.values.byName(status.toSql());
    return _db.listBroadcastModerationQueue().status(sdkStatus).ref().subscribe().map((snap) =>
      snap.data.broadcasts.map((b) => <String, dynamic>{
        'id': b.id.toString(),
        'title': b.title,
        'message': b.message,
        'targetCountry': b.targetCountry,
        'targetRegion': b.targetRegion,
        'targetCity': b.targetCity,
        'status': b.status,
        'moderationStatus': b.moderationStatus.stringValue,
        'flagReason': b.flagReason,
        'targetRole': b.targetRole?.stringValue,
        'targetSectorId': b.targetSector?.id,
        'targetCategoryId': b.targetCategory?.id,
  ...
  Future<void> moderateBroadcast(String id, ModerationStatus status, {String? reason}) async {
    final sdkStatus = sdk.ModerationStatus.values.byName(status.toSql());
    final builder = _db.moderateBroadcast(id: id, status: sdkStatus);
    if (reason != null) builder.reason(reason);
    await builder.execute();
  }

  @override
  Future<List<models.WalletTransactionModel>> getFinanceStats({DateTime? start, DateTime? end}) async {
    final builder = _db.getFinanceStats();
    if (start != null) builder.start(sdk.Timestamp.fromDateTime(start));
    if (end != null) builder.end(sdk.Timestamp.fromDateTime(end));

    final result = await builder.execute();
    return result.data.walletTransactions.map((t) => models.WalletTransactionModel(
      id: t.id.toString(),
      userId: t.user.id,
      amount: t.amount,
      type: TransactionType.parse(t.type.stringValue),
  ...
  Future<List<Map<String, dynamic>>> getAnalyticsSnapshots({DateTime? start, DateTime? end}) async {
    ...
    if (start != null) builder.start(sdk.Date.fromDateTime(start));
    if (end != null) builder.end(sdk.Date.fromDateTime(end));
    
    final result = await builder.execute();
    return result.data.analyticsSnapshots.map((s) => {
      'id': s.id.toString(),
      'date': s.date.toDateTime(),
  ...
  Future<void> createAnalyticsSnapshot(Map<String, dynamic> data) async {
    await _db.createAnalyticsSnapshot(
      date: sdk.Date.fromDateTime(data['date']),
  ...
  Future<void> incrementServiceMetric(String serviceId, {int views = 0, int contacts = 0, int shares = 0}) async {
    await _db.incrementServiceMetric(
      serviceId: serviceId,
      views: views,
      contacts: contacts,
      shares: shares,
  ...
  Stream<List<Map<String, dynamic>>> listActiveBroadcasts({String? country, UserRole? role}) {
    var builder = _db.listActiveBroadcasts();
    if (country != null) builder = builder.country(country);
    if (role != null) builder = builder.role(sdk.UserRole.values.byName(role.toSql()));
