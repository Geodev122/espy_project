...
class _AuthScreenState extends State<AuthScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildCardHeader(AppLocalizations l10n) {
    String subtitle = widget.initialRole?.toUpperCase() ?? '';
    if (subtitle == 'VISITOR') subtitle = 'NETWORK ACCESS';
    ...
    return Column(children: [
      ...
      if (_isSignUp && subtitle.isNotEmpty) ...[
         const SizedBox(height: 4),
         Text(subtitle, style: GoogleFonts.cinzel(fontSize: 10, fontWeight: FontWeight.w900, color: EspyTheme.royalBlue, letterSpacing: 2)),
      ],
    ]);
  }
  ...
  Future<void> _onGoogleSignIn() async {
    final authService = Provider.of<AuthService>(context, listen: false);
    try {
      final credential = await authService.signInWithGoogle(initialRole: widget.initialRole);
      if (credential != null && mounted) {
        await authService.fetchUserData();
        if (mounted) {
          FocusScope.of(context).unfocus();
