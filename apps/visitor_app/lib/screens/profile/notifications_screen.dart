...
class NotificationsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: EspyTheme.background, // Kept light brand color
      appBar: AppBar(
                ...
                if (notifications.isEmpty) {
                  return Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.notifications_none_rounded, size: 64, color: EspyTheme.cognac.withValues(alpha: 0.1)),
                            ...
                            children: [
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: isGlobal ? EspyTheme.gold.withValues(alpha: 0.1) : EspyTheme.cognac.withValues(alpha: 0.1),
                                  shape: BoxShape.circle,
                                ),
                                child: Icon(
                                  isGlobal ? Icons.campaign_rounded : Icons.notifications_active_rounded,
                                  color: isGlobal ? EspyTheme.gold : EspyTheme.cognac,
                                  size: 20,
                                ),
                              ),
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        Text(
                                          ...
                                          style: GoogleFonts.cinzel(fontWeight: FontWeight.w900, fontSize: 13, color: EspyTheme.cognac),
                                        ),
                                        if (isGlobal)
                                          Text(
                                    ...
                                    Text(
                                      n['message'] ?? n['content'] ?? '',
                                      style: GoogleFonts.lora(fontSize: 12, color: EspyTheme.cognac.withValues(alpha: 0.6), height: 1.5),
                                    ),
                                    const SizedBox(height: 12),
                                    Text(
                                      l10n.receivedByProtocol.toUpperCase(),
                                      style: GoogleFonts.cinzel(fontSize: 7, fontWeight: FontWeight.bold, color: EspyTheme.cognac.withValues(alpha: 0.3), letterSpacing: 1),
                                    ),
                                  ],
                                ),
                              ),
