...
class _ProfessionalWizardState extends State<ProfessionalWizard> {
  ...
  Future<void> _loadInitialData() async {
    ...
    if (data != null) {
      setState(() {
        _nameController.text = data['fullNameEn'] ?? auth.user?.displayName ?? '';
        _bioController.text = data['bio'] ?? '';
        _bioArController.text = data['bio_ar'] ?? '';
        _whatsappCodeController.text = data['whatsapp_code'] ?? '+961';
        _whatsappNumberController.text = data['whatsapp_number'] ?? '';
        _specializationController.text = data['specialization'] ?? '';
        _specializationArController.text = data['specialization_ar'] ?? '';
        _selectedSectorId = data['sectorId'];
        _selectedCategoryId = data['categoryId'];
        _mainLocation = data['mainLocation'];
          ...
          child: Column(
            ...
            children: [
                  ...
                  return Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButtonFormField<String>(
                        ...
                        items: sectors.map<DropdownMenuItem<String>>((sector) {
                          final name = (isAr ? sector['name_ar'] : sector['name_en']) ?? sector['name_en'] ?? 'OTHER';
                        ...
                        decoration: InputDecoration(
                          hintText: l10n.selectSector.toUpperCase(),
              ...
              if (_selectedSectorId != null) ...[
                const SizedBox(height: 24),
                _buildFieldLabel(l10n.professionalSector.toUpperCase()),
                StreamBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final filtered = allCats.where((c) => 
                      c['parent_id'].toString() == _selectedSectorId.toString()
                    ).toList();
                    ...
                    return Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButtonFormField<String>(
                          ...
                          items: filtered.map<DropdownMenuItem<String>>((cat) {
      ...
      case 1:
        return _buildStepFade(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildStepTitle(l10n.professionalExpertise),
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Describe your clinical background and areas of expertise...',
                ),
              ),
              const SizedBox(height: 24),
              ...
              TextFormField(
                ...
                textDirection: TextDirection.rtl,
                decoration: const InputDecoration(
                  hintText: 'صف خلفيتك السريرية ومجالات خبرتك...',
                ),
              ),
              const SizedBox(height: 32),
              DocumentPicker(
          ...
          child: Column(
            children: [
              ...
              PremiumCard(
                ...
                child: Column(
                  children: [
                    const Icon(Icons.verified_user_rounded, size: 64, color: EspyTheme.teal),
                    const SizedBox(height: 32),
  ...
  Future<void> _handleNext() async {
    ...
    if (_currentStep < 3) {
      if (_currentStep == 0) {
        if (_nameController.text.isEmpty || _selectedSectorId == null || _selectedCategoryId == null) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.nameSectorCategoryRequired)));
          return;
        }
      }
  ...
  Future<void> _submitProfile() async {
    ...
    try {
      ...
      await _firestore.createProfessionalProfile(uid, {
        ...
        'role': 'professional',
        'visibilityNodes': 0, // Starts with zero visibility
        'serviceSlots': 0,   // Starts with zero slots
        'practicePins': 0,   // Starts with zero pins
        'membershipTier': 0,
        'isApproved': false,
        'isActive': true, // Account is active, but visibility is zero
        'hasProfile': true,
      ...
      // Refresh auth state to trigger MainGate rebuild (Dashboard redirection)
