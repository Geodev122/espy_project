...
import 'package:espy_pro/screens/services/service_listing_wizard.dart';
...
class _ServiceManagerScreenState extends State<ServiceManagerScreen> {
  ...
  Widget _buildServiceTile(Map<String, dynamic> service, int totalSlots, int usedSlots) {
    ...
    final expiry = service['visibilityExpiresAt'] != null 
        ? (service['visibilityExpiresAt'] is Timestamp 
            ? (service['visibilityExpiresAt'] as Timestamp).toDate() 
            : DateTime.tryParse(service['visibilityExpiresAt'].toString()))
        : null;
                ...
                children: [
                  ...
                  if (isAllocated && !isExpired)
                    ...
                    Container(
                      ...
                      child: Text("EXPIRED - RENEW IN VAULT", style: GoogleFonts.cinzel(fontSize: 7, fontWeight: FontWeight.bold, color: EspyTheme.error)),
                    )
                  else
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                  ...
                  if (expiry != null)
                    Padding(
                      ...
                      child: Text("Expires: ${DateFormat('dd MMM yy').format(expiry)}", style: GoogleFonts.lora(fontSize: 9, color: isExpired ? EspyTheme.error : Colors.black38)),
                    ),
                ],
              ),
            ),
            Switch(
              value: isAllocated && !isExpired,
